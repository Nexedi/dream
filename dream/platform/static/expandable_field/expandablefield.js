...
/*jslint nomen: true */
(function(window, rJS, RSVP, Handlebars, initGadgetMixin, loopEventListener) {
    "use strict";
    /////////////////////////////////////////////////////////////////
    // Handlebars
    /////////////////////////////////////////////////////////////////
    ...
    function _handleSelectChange() {
        ...
        var gadget = this, oneOf_list, i, prop_name = gadget.props.definition.property_def.title, select = gadget.props.element.getElementsByTagName("select")[0], update_name = select.options[select.selectedIndex].value;
        if (!(update_name === prop_name)) {
            prop_name = update_name;
            // change gadget.props.definition 
            for (i = 0; i <= gadget.props.options.property_definition.allOf.length - 1; i += 1) {
                if (gadget.props.options.property_definition.allOf[i].oneOf) {
                    oneOf_list = gadget.props.options.property_definition.allOf[i].oneOf;
                    break;
                }
            }
            for (i = 0; i <= oneOf_list.length - 1; i += 1) {
                if (oneOf_list[i].title === prop_name) {
    ...
    function waitForListFieldSelection(gadget) {
        ...
        return loopEventListener(element, "change", false, handleSelectChange.bind(gadget));
    }
    ...
    gadget_klass.declareMethod("render", function(options, node_id) {
        ...
        // id
        var gadget = this, queue, index, len, ind, oneOf_list, req_props_sdict = {}, ab_title, ab_definition, w, value, prop_name, prop_definition, string_value;
        ...
        queue = new RSVP.Queue().push(function() {
            ...
            for (index = 0; index <= len - 1; index += 1) {
                ...
                console.log(gadget.props.options.property_definition.allOf[index]);
                if (gadget.props.options.property_definition.allOf[index].oneOf) {
                    /* adding various hidden list fields */
                    oneOf_list = gadget.props.options.property_definition.allOf[index].oneOf;
               ...
               get a property definition for object-type fields
                req_props_sdict */
                    ...
                    // XXX add a listfield
                    if (Object.keys(gadget.props.options.property_definition.allOf[index].properties).length !== 1) {
        ...
        return RSVP.Queue().push(function() {
            ...
            for (index = 0; index <= keys.length - 1; index += 1) {
                ...
                if (typeof results[Object.keys(results)[0]][keys[index]] === "object") {
                    // hold the keys of that object
                    sub_keys = Object.keys(results[Object.keys(results)[0]][keys[index]]);
                    if (keys[index] === gadget.props.definition.property_def.title) {
                        if (gadget.props.definition.property_def.properties) {
                            // find the allowed keys according to the prop_definition
                            allowed_sub_keys = Object.keys(gadget.props.definition.property_def.properties);
        ...
        var gadget = this, ind, sel_ind, sought_child;
        return RSVP.Queue().push(function() {
            for (sel_ind = 0; sel_ind <= gadget.props.element.children.length - 1; sel_ind += 1) {
